...
$.ui.fancytree._FancytreeClass.prototype.testMethod = function (data, columns, LISTATREE,listaHiden) {
   //debugger;
   ...
   if (data.node.arg13 != null) LISTATREE = data.node.arg13;

   //$tdList.eq(1).text(node.getIndexHier()).addClass("alignRight");
   if (node.isFolder()) {
      $tdList.eq(0)
         .prop("colspan", columns)
         .nextAll().remove();
   }
   //debugger;
   for (var i = 1; i <= columns; i++) {
      if (node.key.indexOf("_") == -1) {
         //if (node.key < LISTATREE.length) {
         switch (LISTATREE[i]) {
            case "<select />":
               $select = $("<select id=" + "node" + node.key + "col" + i + "dllSelect" + "/>")
               $("<option />", { text: "-Seleccionar-" }).appendTo($select);
               $tdList.eq(i).html($select);
               break;
            default:
               $tdList.eq(i).html("<span class='fancytree-node'><span class = 'fancytree-title' tabindex = '0'>" + LISTATREE[i] + "</span></span>");
         }
         

      } else {
         $tdList.eq(i).html("<span class = 'fancytree-title' tabindex = '0'>" + "" + "</span>");
      }

      if (listaHiden) {
         if (listaHiden.indexOf(i) !== -1)
            $tdList.eq(i).attr("hidden", true);
      }
      //}
   }
