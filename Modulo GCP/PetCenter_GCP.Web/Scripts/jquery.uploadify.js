...
(function ($) {
        ...
        init: function (options, swfUploadOptions) {
            ...
            return this.each(function () {
                ...
                var settings = $.extend({
                    ...
                    buttonText: 'Seleccionar Archivo',     // The text to use for the browse button
                    ...
                    successTimeout: 360,                 // The number of seconds to wait for Flash to detect the server's response after the file has finished uploading
                    uploadLimit: 0,                  // The maximum number of files you can upload
                    width: 110,                // The width of the browse button
        ...
        cancel: function (fileID, supressEvent) {
            ...
            this.each(function () {
                ...
                if (args[0]) {
                    ...
                    if (args[0] == '*') {
                        ...
                        $('#' + settings.queueID).find('.uploadify-queue-item').each(function () {
                            ...
                            $(this).find('.data').removeClass('data').html(' - Cancelado');
                            $(this).find('.uploadify-progress-bar').remove();
                            $(this).delay(1000 + 100 * delay).fadeOut(50, function () {
                                $(this).remove();
                                $.colorbox.resize();
                            });
                        });
                        ...
                        for (var n = 0; n < args.length; n++) {
                            ...
                            $('#' + args[n]).find('.data').removeClass('data').html(' - Cancelado');
                            $('#' + args[n]).find('.uploadify-progress-bar').remove();
                            $('#' + args[n]).delay(0).fadeOut(50, function () {
                                $(this).remove();
                                $.colorbox.resize();
                            });
                        }
                    ...
                    $item.find('.data').removeClass('data').html(' - Cancelado');
                    $item.find('.uploadify-progress-bar').remove();
                    $item.delay(1000).fadeOut(50, function () {
                        $(this).remove();
                        $.colorbox.resize();
                    });
                }
                ...
                for (var i = 0; i < arrayTemp.length; i++) {
                    if (arrayTemp[i].innerHTML == " - Error")
                        cant++;
                }
                if (cant == 0)
                    $("#hdFlagSuccess").val("0");
    ...
    var handlers = {
        ...
        onSelect: function (file) {
            // Load the swfupload settings
            var settings = this.settings;
            //var r = confirm('¿Está seguro de Cargar el Archivo "' + file.name + '"?');
            //if (!r) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    //var replaceQueueItem = confirm('El archivo "' + file.name + '" ya ha sido Subido.\n Quiere reemplazarlo?');
                    //if (!replaceQueueItem) {
                    //    this.cancelUpload(file.id);
        ...
        onUploadSuccess: function (file, data, response) {
            ...
            if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
                if (result.success)
                    $('#' + file.id).find('.data').html(' - Administrado(s) Cargado(s)');
                else
