    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
                ...
                dragEnd: function () {
                    ...
                    if (this.resizing) {
                        ...
                        if (p.forceFit === true) {
                            ...
                            /*FIX BY EFP para Resizing de Columnas*/
                            p.tblwidth = this.newWidth > 9999 ? p.tblwidth : this.newWidth || p.tblwidth;
            ...
            formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata) {
                ...
                if (cm.upperCase != undefined) {
                    if (cm.upperCase) result += "text-transform:uppercase;";
                    else result += "text-transform:none;";
                }
                if (rowInd === 0) {
            ...
            addCell = function (rowId, cell, pos, irow, srvr, rdata) {
                ...
                var td = "<td role=\"gridcell\" " + prp + ">" + v + "</td>";
                        ...
                        case "script":
                            $.ajax($.extend({
                                ...
                                error: function (xhr, st, err) {
                                    if (ts.p.dynamicGrid)
                                        $("#" + ts.id).jqGrid("clearGridData", true);
            ...
            setPager = function (pgid, tp) {
                ...
                $('.ui-pg-selbox', "#" + pgcnt).bind('change', function () {
                    /* Fix By EFP Change */
                    var trs = $("#" + ts.id).find("tr");
                    for (var i = 0; i < trs.length; i++) {
                        if (trs[i].id != "") {
                            if ($("#" + ts.id).find("#" + trs[i].id).attr('editable') == "1") {
                                $('.ui-pg-selbox', "#" + pgcnt).val(ts.p.rowNum);
                                return;
                            }
                        }
                    }
                    ...
                    populate();
                    if (ts.p.enableSelection) {
                        setTimeout(function () {
                            var idTr = $("#" + ts.p.id).find("tr").eq(1).attr("id");
                            $("#" + ts.p.id).jqGrid('setSelection', idTr, true);
                            $("#" + ts.p.id).focus();
                ...
                if (ts.p.pgbuttons === true) {
                    ...
                    $("#first" + $.jgrid.jqID(tp) + ", #prev" + $.jgrid.jqID(tp) + ", #next" + $.jgrid.jqID(tp) + ", #last" + $.jgrid.jqID(tp)).click(function () {
                        get_DialogFocus();
                        /*Fix Pagination By EFP */
                        var trs = $("#" + ts.p.id).find("tr");
                        for (var i = 0; i < trs.length; i++) {
                            if (trs[i].id != "") {
                                if ($("#" + ts.p.id).find("#" + trs[i].id).attr('editable') == "1") {
                                    return;
                                }
                            }
                        }
                ...
                if (ts.p.pginput === true) {
                    $('input.ui-pg-input', "#" + pgcnt).keypress(function (e) {
                        ...
                        if (key === 13) {
                            /*Fix By EFP para no cerrar el Inline Edition al Dar Enter al Input de Pagination*/
                            var trs = $("#" + ts.id).find("tr");
                            for (var i = 0; i < trs.length; i++) {
                                if (trs[i].id != "") {
                                    if ($("#" + ts.id).find("#" + trs[i].id).attr('editable') == "1") {
                                        $('input.ui-pg-input', "#" + pgcnt).val(ts.p.page);
                                        return;
                                    }
                                }
                            }
                            ...
                            ts.p.page = ($(this).val() == 0 || ts.p.lastpage < $(this).val()) ? ts.p.page : $(this).val();
                            populate();
                            if (ts.p.enableSelection) {
                                setTimeout(function () {
                                    var idTr = $("#" + ts.p.id).find("tr").eq(1).attr("id");
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                /* Fix BY EFP para el Pivoteo dinamico*/
                //debugger;
                var colname = ts.p.dynamicGrid ? (ts.p.colNames[i].split('||').length > 1 ? ts.p.colNames[i].split('||')[1] : ts.p.colNames[i].split('||')[0]) : ts.p.colNames[i];

                var tooltip = ts.p.headertitles ? (" title=\"" + $.jgrid.stripHtml(ts.p.colNames[i]) + "\"") : "";
                thead += "<th id='" + ts.p.id + "_" + ts.p.colModel[i].name + "' role='columnheader' class='ui-state-default ui-th-column ui-th-" + dir + "'" + tooltip + ">";
                idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
                thead += "<div id='jqgh_" + ts.p.id + "_" + ts.p.colModel[i].name + "' " + tdc + ">" + colname;
            ...
            $(ts).before(grid.hDiv).click(function (e) {
                ...
                /*Fix by EFP para setear la primera fila en caso no haya ninguno seleccionado */
                //setTimeout(function () { $(ts).focus(); }, 300);
                if (ts.p.enableSelection) {
                    setTimeout(function () { if (ts.p.selrow == null) $(ts).focus(); }, 300);
                }
                return false;
            })
            .dblclick(function (e) {
    ...
    $.jgrid.extend({
        ...
        setSelection: function (selection, onsr, e) {
            return this.each(function () {
                ...
                /*Fix By EFP set Selection for Editable Attributes */
                var state = false;
                if (!$t.p.treeGrid) {
                    if (!$t.p.dynamicGrid) {
                        ...
                        for (var i = 0; i < trs.length; i++) {
                            if (trs[i].id != "") {
                                if ($($t).find("#" + trs[i].id).attr('editable') == "1") {
                                    $($t).find("#" + trs[i].id).addClass("ui-state-highlight");
                                    state = true;
                                }
                                else {
                                    if (state) {
                                        $($t).find("#" + trs[i].id).removeClass("ui-state-highlight");
                                    }
                                }
                            }
                        }
                ...
                if (!$t.p.multiselect) {
                    if (pt.className !== "ui-subgrid") {
                        if ($t.p.selrow !== pt.id) {
                            ...
                            stat = false;
                            /*Fix by EFP para el trabajo de grilla en memoria ya que el setSelection se pega */
                            if (!$t.p.treeGrid) {
                                if ($(pt).attr("tabindex") == "-1" || $t.p.selrow == pt.id) {
                                    //$($t.rows.namedItem($t.p.selrow)).removeClass("ui-state-highlight").attr({ "aria-selected": "false", "tabindex": "-1" });
                                    var trs = $($t).find("tbody tr");
                                    for (var i = 0; i < trs.length; i++) {
                                        $("#" + trs[i].id).removeClass("ui-state-highlight").attr({ "aria-selected": "false", "tabindex": "-1" });
                                    }
                                    setTimeout(function () { $(pt).addClass("ui-state-highlight").attr({ "aria-selected": "true", "tabindex": "0" }); }, 0);
                                    if (fid) {
                                        $("#" + $.jgrid.jqID($t.p.selrow), "#" + $.jgrid.jqID(fid)).removeClass("ui-state-highlight");
                                        $("#" + $.jgrid.jqID(selection), "#" + $.jgrid.jqID(fid)).addClass("ui-state-highlight");
                                    }
        ...
        resetSelection: function (rowid) {
            return this.each(function () {
                ...
                if (rowid !== undefined) {
                    ...
                    /* Fix by EFP para una sola selección de fila */
                    //else if (t.p.selrow == null) {
                    //    var trs = $("#" + $.jgrid.jqID(t.p.id) + " tbody tr");
                    //    for (var i = 0; i < trs.length; i++) {
                    //        if (trs[i].id != "")
                    //            $("#" + $.jgrid.jqID(t.p.id)).find("#" + trs[i].id).removeClass("ui-state-highlight");
                    //    }
                    //}
                    /*Fin Fix*/
                } else {
        ...
        bindKeys: function (settings) {
            /* Modificado By EFP
                Se agregó el keypress para suprimir
            */
            var o = $.extend({
                onEnter: null,
                onpace: null,
            ...
            return this.each(function () {
                ...
                $($t).keydown(function (event) {
                    ...
                    if (target) {
                        /*Fix by EFP para validar los editables */
                        var idPager = $t.p.pager.substring(1);
                        var estadoGrid = true;

                        var trs = $("#" + $t.p.id).find("tr");
                        for (var i = 0; i < trs.length; i++) {
                            if (trs[i].id != "") {
                                if ($("#" + $t.p.id).find("#" + trs[i].id).attr('editable') == "1")
                                    estadoGrid = false;
                            }
                        }
                        /*Fin fix*/
                        ...
                        if (event.keyCode === 37 || event.keyCode === 38 || event.keyCode === 39 || event.keyCode === 40) {
                            ...
                            if (event.keyCode === 38) {
                                ...
                                if (r) {
                                    /* Parche By EFP 
                                        KeyBoardNavigation con subgrid
                                        || !$(r).hasClass('jqgrow')
                                    */
                                    if ($(r).is(":hidden")) {
                                        while (r) {
                            ...
                            if (event.keyCode === 40) {
                                ...
                                if (r) {
                                    /* Parche By EFP 
                                        KeyBoardNavigation with subgrid
                                        || !$(r).hasClass('jqgrow')
                                    */
                                    if ($(r).is(":hidden")) {
                                        while (r) {
                            ...
                            /*Fix by EFP para Paginación con teclas */
                        else if (event.keyCode === 36) {
                            if (estadoGrid) {
                                if ($("#first_" + idPager)[0].className !== "ui-pg-button ui-corner-all ui-state-disabled") {
                                    $("#first_" + idPager).click();
                                    //setTimeout(function () {
                                    //    var idTr = $("#" + $t.p.id).find("tr").eq(1).attr("id");
                                    //    $("#" + $t.p.id).jqGrid('setSelection', idTr, true);
                                    //    $($t).focus();
                                    //}, 300);
                                }
                            }
                            ...
                            if (estadoGrid) {
                                if ($("#last_" + idPager)[0].className !== "ui-pg-button ui-corner-all ui-state-disabled") {
                                    $("#last_" + idPager).click();
                                    //setTimeout(function () {
                                    //    var idTr = $("#" + $t.p.id).find("tr").eq(1).attr("id");
                            ...
                            if (estadoGrid) {
                                if ($("#next_" + idPager)[0].className !== "ui-pg-button ui-corner-all ui-state-disabled") {
                                    $("#next_" + idPager).click();
                                    //setTimeout(function () {
                                    //    var idTr = $("#" + $t.p.id).find("tr").eq(1).attr("id");
                                    //    $("#" + $t.p.id).jqGrid('setSelection', idTr, true);
                            ...
                            if (estadoGrid) {
                                if ($("#prev_" + idPager)[0].className !== "ui-pg-button ui-corner-all ui-state-disabled") {
                                    $("#prev_" + idPager).click();
                                    //setTimeout(function () {
                                    //    var idTr = $("#" + $t.p.id).find("tr").eq(1).attr("id");
                                    //    $("#" + $t.p.id).jqGrid('setSelection', idTr, true);
        ...
        getEditableRowId: function (rowid) {
            var $t = this[0];
            var rowId = 0;
            var trs = $("#" + $t.id).find("tr");
            for (var i = 0; i < trs.length; i++) {
                if (trs[i].id != "") {
                    if ($("#" + $t.id).find("#" + trs[i].id).attr('editable') == "1") {
                        rowId = $("#" + $t.id).find("#" + trs[i].id).attr("id");
                    }
                }
            }
            return rowId;
        },
        getDataRowsNoEditablesLength: function (rowid) {
            var $t = this[0];
            var contador = 0;
            var trs = $("#" + $t.id).find("tr");
            for (var i = 0; i < trs.length; i++) {
    ...
    */
    ...
    $.extend($.jgrid, {
        ...
        info_dialog: function (caption, content, c_b, modalopt) {
            ...
            //var cnt = "<div id='info_id' style='display:none;'>";
        ...
        // Form Functions
        createEl: function (eltype, options, vl, autowidth, ajaxso, upperCase) {
            /* FIX BY EFP upperCase Transform xD */
            var elem = "", $t = this;
            ...
            */
            ...
            this.createTableRowForRule = function (rule, group) {
                ...
                ruleFieldSelect.bind('change', function () {
                    ...
                    var elm = $.jgrid.createEl.call($t, cm.inputtype, cm.searchoptions, "", true, that.p.ajaxSelectOptions, true, cm.upperCase);
                ...
                var ruleDataInput = $.jgrid.createEl.call($t, cm.inputtype, cm.searchoptions, rule.data, true, that.p.ajaxSelectOptions, true, cm.upperCase);
                ...
                function createData(rowid, obj, tb, maxcols) {
                    ...
                    $(obj.p.colModel).each(function (i) {
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && this.editable === true && nm !== 'rn') {
                            ...
                            elc = $.jgrid.createEl.call($t, this.edittype, opt, tmp, false, $.extend({}, $.jgrid.ajaxOptions, obj.p.ajaxSelectOptions || {}), this.upperCase);
    ...
    $.jgrid.extend({
                    ...
                    $('td[role="gridcell"]', ind).each(function (i) {
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                            ...
                            if (cm[i].editable === true) {
                                ...
                                if (tmp === "&nbsp;" || tmp === "&#160;" || (tmp.length === 1 && tmp.charCodeAt(0) === 160)) { tmp = ''; }
                                var elc = $.jgrid.createEl.call($t, cm[i].edittype, opt, tmp, true, $.extend({}, $.jgrid.ajaxOptions, $t.p.ajaxSelectOptions || {}), cm[i].upperCase);
                                $(elc).addClass("editable");
                                if (treeg) { $("span:first", this).append(elc); }
                                else {
                                    /* Agregado By EFP para las grillas dinamicas con Inline Edition */
                                    if ($t.p.dynamicGrid) {
                                        //debugger;
                                        if (cm[i].name.split('||').length > 1) {
                                            var arrayContent = opt.id.split('||');
                                            var rowInpuntId = arrayContent[0].split('_');
                                            var ColdModelName = cm[i].name.split('||');
                                            elc.id = rowInpuntId[0] + "_" + ColdModelName[1].replace(/\s/g, '').replace(/[\])}[{(]/g, '');
                                            elc.name = rowInpuntId[0] + "_" + ColdModelName[1].replace(/\s/g, '').replace(/[\])}[{(]/g, '');
                                            if (elc.type == "text") {
                                                elc.style.width = '55px';
                                                //elc.maxlength = "4";
                                            }
                                            var valarray = elc.value.split('||');
                                            elc.setAttribute("key", valarray[0]/*ColdModelName[0]*/);
                                            elc.setAttribute("key2", ColdModelName[0]);
                                            elc.value = valarray[1];
                                            if (elc.addEventListener)
                                                elc.addEventListener("click", function () { $($t).jqGrid("setSelection", rowid, true); }, false);
                                            else
                                                elc.attachEvent('onclick', function () { $($t).jqGrid("setSelection", rowid, true); });
                                        } else {
                                            elc.id = opt.id.replace(/\s/g, '');
                                            elc.name = opt.name.replace(/\s/g, '');
                                        }
                                    }
                                    else {
                                        elc.id = opt.id;
                        ...
                        if (o.keys === true) {
                            ...
                            $(ind).bind("keydown", function (e) {
                                ...
                                if (e.keyCode === 13) {
                                    ...
                                    /*Fix by EFP para deshabilitar Enter en algunas cajas de texto en Inline Edition */
                                    if (ta.getAttribute('data-disableEnterKey') == "1") { return true; }
                                    if (ta.getAttribute('data-blur') == "1") {
                                        if ($.dlg("#" + ta.id).length > 0)
                                            $.dlg("#" + ta.id).blur();
                                        else
                                            $("#" + ta.id).blur();
                                    }
                                    /*Fix by EFP para deshabilitar el Enter cuando no está en Inline Edition */
                                    var rowIdTemp = $($t).jqGrid('getGridParam', 'selrow');
                                    if ($($t).find("tr#" + rowIdTemp).attr("editable") === "1") {
                                        if ($($t).jqGrid("saveRow", rowid, o)) {
                                            if ($t.p._inlinenav) {
                                                try {
                                                    $($t).jqGrid('showAddEditButtons');
                                                    /* FIX BY EFP para no perder el foco cuando se maneje la data en memoria */
                                                    if (o.successfunc == null) {
                                                        setTimeout(function () {
                                                            var trs = $("#" + $t.id).find("tr");
                                                            if (trs.length >= 2) {
                                                                $($t).jqGrid("setSelection", trs[1].id, true);
                                                                //$($t).find("#" + trs[1].id).focus();
                                                            }
                                                        }, 0);
                                                    }
                                                    else
                                                        setTimeout(function () { $($t).jqGrid("setSelection", 1, true); $($t).focus(); }, 0);
                                                } catch (eer2) { }
                                            }
                                        }
                                    }
                                    else if ($($t).find("tr#" + rowid).attr("editable") === "1") {
                                        $t.p.selrow = rowid;
                                        setTimeout(function () {
                                            if ($($t).jqGrid("saveRow", rowid, o)) {
                                                if ($t.p._inlinenav) {
                                        ...
                                        $($t).jqGrid("bindKeys");
                                        $($t).jqGrid("restoreRow", rowid, o.afterrestorefunc);
                                        $($t).jqGrid("setSelection", rowIdTemp, true);
                                        //$($t).focus();
                                    }
                                    /*En Fix*/
                                    return false;
                ...
                $('td[role="gridcell"]', ind).each(function (i) {
                    ...
                    if (nm !== 'cb' && nm !== 'subgrid' && cm.editable === true && nm !== 'rn' && !$(this).hasClass('not-editable-cell')) {
                        switch (cm.edittype) {
                            ...
                            case 'select':
                                /* Modificado By EFP
                                    Se modifico para que valide si existe la propiedad editoptions
                                    para que pueda obtener los valores de los combos en un Inline Edition
                                */
                                if (cm.hasOwnProperty('editoptions')) {
            ...
            return this.each(function () {
                ...
                if (fr >= 0) {
                    ...
                    if ($("#" + $.jgrid.jqID(rowid), "#" + $.jgrid.jqID($t.p.id)).hasClass("jqgrid-new-row")) {
                        ...
                        /* Agregado By EFP
                           Fix para Los Botones del Inline */
                    else {
                        setTimeout(function () {
                            $($t).jqGrid('showAddEditButtons');
                        }, 0);
                ...
                //if ($(this.p.pager).find(".ui-pg-selbox").length > 0)
                //    $(this.p.pager).find(".ui-pg-selbox")[0].selectedIndex = 0;
                /* FIX BY EFP set Selection*/
                if (isNaN($t.p.selrow)) {
                    setTimeout(function () {
                        //var existe = $($t).find("#" + $t.p.idPrefix + "1").length;
                        //if (existe > 0) {
                        //    $($t).jqGrid('setSelection', $t.p.idPrefix + 1, true);
                        //    $($t).find("#" + $t.p.idPrefix + "1").focus();
                        //} else {
                        //    $($t).jqGrid('setSelection', rowid, true);
                        //    $($t).find("#" + rowid).focus();
                        //}
                        /* Fix By EFP */
                        var existe = $($t).find("#" + rowid).length;
                        if (existe > 0) {
                            $($t).jqGrid('setSelection', rowid, true);
                            $($t).find("#" + rowid).focus();
                        } else {
        ...
        addRowCustom: function (p) {
            ...
            return this.each(function () {
                ...
                if (p.useFormatter) {
                    ...
                    if (p.addRowParams.url == undefined)
                        $($t).jqGrid('editRow', p.rowID, x);
                    else
            ...
            return this.each(function () {
                ...
                if (p.useFormatter) {
                    ...
                    if (p.addRowParams.url == undefined)
                        ...
                        $($t).jqGrid('editRow', p.rowID, p.addRowParams);
                    $($t).jqGrid('setSelection', p.rowID, true);
                }
            });
        },
        inlineNav: function (elem, o) {
            ...
            return this.each(function () {
                ...
                if (o.edit) {
                    $($t).jqGrid('navButtonAdd', elem, {
                        ...
                        onClickButton: function () {
                            ...
                            if (sr) {
                                if ($($t).jqGrid('getRowData').length > 0) {
                    ...
                    $t.p.beforeSelectRow = function (id, stat) {
                        ...
                        if ($t.p.savedRow.length > 0 && $t.p._inlinenav === true && (id !== $t.p.selrow && $t.p.selrow !== null)) {
                            if ($t.p.selrow === o.addParams.rowID) {
                                ...
                                //var rowidTemp;
                                var trs = $("#" + this.p.id).find("tr");
                                for (var i = 0; i < trs.length; i++) {
                                    if (trs[i].id != "") {
                                        if ($("#" + this.p.id).find("#" + trs[i].id).attr('editable') == "1") {
                                            estado = true;
                                        }
                                    }
                                }
                ...
                /*Inicio Fix By JJJ inlineEdit Subgrid 24/02/2014*/
                if (o.saveCustom) {
                    $($t).jqGrid('navButtonAdd', elem, {
                        caption: o.savetext || '',
                        title: o.savetitle || 'Save row',
                        ...
                        onClickButton: function () {
                            ...
                            if ($t.p.savedRow.length === 0) {
                                sr = $($t).find('tr[editable="1"]').get(0).id
                                $($t).jqGrid('setSelection', sr, true);
                            }
                            else {
                                sr = $t.p.savedRow[0].id;
                            }
                ...
                /*Fin Fix By JJJ inlineEdit Subgrid 24/02/2014*/
            });
        },
        ...
        editCell: function (iRow, iCol, ed) {
            return this.each(function () {
                ...
                if (cm.editable === true && ed === true && !cc.hasClass("not-editable-cell")) {
                    ...
                    var elc = $.jgrid.createEl.call($t, cm.edittype, opt, tmp, true, $.extend({}, $.jgrid.ajaxOptions, $t.p.ajaxSelectOptions || {}), cm.upperCase);
    ...
    $.jgrid.extend({
        ...
        groupingRender: function (grdata, colspans) {
            return this.each(function () {
                ...
                //--- Function Add by Omar By jqPivot
                ...
                var sumreverse = $.makeArray(grp.groupSummary), mul; //--- Add Omar for Pivot
                sumreverse.reverse();
                ...
                $.each(grp.groups, function (i, n) {
                    ...
                    //--- This line is commented by Omar by Pivot
                    //str += "<tr id=\"" + hid + "\" role=\"row\" class= \"ui-widget-content jqgroup ui-row-" + $t.p.direction + " " + clid + "\"><td style=\"padding-left:" + (n.idx * 12) + "px;" + "\" colspan=\"" + colspans + "\">" + icon + $.jgrid.template(grp.groupText[n.idx], gv, n.cnt, n.summary) + "</td></tr>";

                    //--- Lines add by Omar by Pivot Header -- Start
                    var grpTextStr = '';
                    ...
                    if (grp.groupSummaryPos[n.idx] === 'header') {
                        ...
                        str += "<tr id=\"" + hid + "\"" + (grp.groupCollapse && n.idx > 0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"ui-widget-content jqgroup ui-row-" + $t.p.direction + " " + clid + "\">" +
                            "<td style=\"padding-left:" + (n.idx * 12) + "px;" + "\"" + mul + ">" + icon + grpTextStr + "</td>";
                    ...
                    //--- Lines add by Omar by Pivot Header -- End
                    ...
                    if (leaf) {
                        ...
                        //--- Lines add by Omar by Pivot Summary Footer -- Start
                        if (grp.groupSummaryPos[n.idx] !== 'header') {
                        ...
                        //--- Lines add by Omar by Pivot Summary Footer -- End
                    }
                });
